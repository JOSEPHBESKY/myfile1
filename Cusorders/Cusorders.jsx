...
function Cusorders(props) {
  ...
  console.log(driverCost, 'driverCost')
          ...
          <Stack direction='row' display='flex' justifyContent='space-between'>
            <Stack direction='row' alignItems='center' sx={{ width: '50%', height: '40px', backgroundColor: 'white', borderRadius: '10px 10px 0px 0px', position: 'relative', bottom: '20px' }}>
              <Stack alignItems='center' sx={{ height: '35px', backgroundColor: '#898BD6', borderRadius: '0px 0px 10px 10px', marginLeft: '15px', position: 'relative', bottom: '3px' }} >
                <label style={{ color: 'white', fontSize: '16px', fontWeight: 'bold', marginLeft: "10px", marginRight: "10px", position: 'relative', top: '3px' }}>Your Orders</label>
              </Stack>
                      ...
                      <Grid key={index} item xs={12} md={3}  >
                        <Card sx={{ maxWidth: '100%', margin: '15px', borderRadius: '15px' }}>
                          <CardHeader
                            ...
                            title={<label style={{ fontSize: 15, fontWeight: 'bold', color: '#3899EC' }}>{i.BNM}</label>}
                            subheader={<Stack>
                              <label style={{ fontSize: 12 }} >{i.BRCYN}</label>
                              <label className='lblTime' >Order #: {i.ID}</label>
                              <label className='lblTime' >Order Placed On</label>
                              <label className='lblTime' >{i.ODFT}</label>
                          ...
                          <CardContent>
                            <Stack>
                              {i.PS.map((p, ind) => (
                                ...
                                <Stack key={ind} direction='row' spacing={4} alignItems='center' justifyContent='space-between'>
                                  <Stack direction='row' spacing={2} alignItems='center'>
                                    <img src={nonveg} height='15px' width='15px' />
                                    <Stack direction='row' spacing={2}>
                                      <label className='labeDish'>{p.QTY}</label>
                                      <label className='labeDish'>x</label>
                                      <label className='labeDish'>{p.PN}</label>
                                    </Stack>
                                  </Stack>
                                  <Stack direction='row-reverse'>
                                    <label className='labeDish'>₹{(Math.round((p.APP) * 100) / 100).toFixed(2)}</label>
                                  </Stack>
                                </Stack>
                              ))}
...

                            {
                              ...
                              //   <label className='lblAMT' style={{marginLeft:'10px'}}>₹{i.OABT}</label>
                              //   {
                              //     i.SID == 'S0003' && (<Stack  display='flex' >{


                              //        <label className='lblAMT'>₹ {JSON.parse(i.DADD).DF}</label>
                              ...
                              //     )
                              //   }
                              //   <Divider sx={{ marginTop: '0px',fontWeight:'bold',color:'black' }}  />

                              //   <label className='lblAMT'  style={{fontWeight:'bold',color:'blue'}} >₹{i.OABT + JSON.parse(i.DADD).DF }</label>
                              //   </Stack>
                            ...
                            }
                            {
                              ...
                              //             </Stack>}
                              //     /> */}
                              //     <Stack direction='row' alignItems='center' justifyContent='space-between' width='100%' sx={{ marginTop: '5px' }}>
                              //         <label className='sub_Lbl'>Sub Total</label>
                              //         <label className='sub_Lbl'>₹ {(Math.round((subTotal) * 100) / 100).toFixed(2)}</label>
                              //     </Stack>
                              // </ListItem>

                              // {driver != '' && DeliveryPrice > 0 ?
                              //     <ListItem alignItems="flex-start">
                              //         <Stack direction='row' alignItems='center' justifyContent='space-between' width='100%' >
                              //             <label className='sub_Lbl'>Delivery</label>
                              //             {/* <label className='sub_Lbl'>₹ {(Math.round((subTotal) * 100) / 100).toFixed(2)}</label> */}
                              //             <label className='sub_Lbl'>{'₹ ' + DeliveryPrice}</label>
                              //         </Stack>
                              //     </ListItem>
                            }

                            {/* <Stack direction='row' sx={{ marginTop: '10px', color: '' }} > */}
                            <Stack sx={{ marginTop: '30px', color: '', cursor: 'pointer' }} direction='row' alignItems='center' justifyContent='space-between'>
                              <label className='lblAMT' style={{ borderBottom: '1px solid #3899EC', color: '#3899EC', cursor: 'pointer' }} onClick={() => HandleNewTap(i.ID, i.CID)}  >view order</label>
                              {i.DYRN != '' ?
                                <label style={{ color: '#3899EC', fontWeight: 'bold', fontSize: 14 }}>Table No {i.DYRN}</label> : <></>}
                            </Stack>
                            {/* </Stack> */}
                            <Divider sx={{ marginTop: '0px' }} />{
                              i.SID != 'S0003' && (<Stack marginTop={5} sx={{ fontWeight: 'bold' }} > {
                                i.OSN == 'Canceled' ? <label style={{ color: 'red' }}  >CANCELED</label> :
                                  i.OSN == 'Delivered' ? <label style={{ color: 'green' }} >Delivered</label> :
                                    i.OSN == 'Accepted' ? <label style={{ color: '#3899EC' }} >ACCEPTED</label> :
                                      i.OSN == '' ? <label style={{ color: '#3899EC' }} >Ready</label> :
                                        i.OSN == 'PaymentFailed' ? <Stack direction='row' alignItems='center' justifyContent='space-between' ><label style={{ color: 'red' }}  >Payment Failed</label>
                                          <label style={{ color: 'blue', borderBottom: '1px solid blue', }} onClick={() => startOnlinePayment(i.ID)} >Payment Retry  </label>

                                        </Stack> : i.OSN == 'Payment Pending' ? <Stack direction='row' alignItems='center' justifyContent='space-between' ><label style={{ color: 'orange' }}  >Payment Pending</label>
                                          <label style={{ color: 'blue', borderBottom: '1px solid blue', }} onClick={() => startOnlinePayment(i.ID)}  >Payment Retry</label>

                                        </Stack> :


                                          <label style={{ color: '#3899EC' }} >{i.OSN}</label>
                              }
                              </Stack>)}{
                              i.SID == 'S0003' && (
                                <Stack marginTop={5} sx={{ fontWeight: 'bold' }} > {
                                  i.OSN == 'Canceled' ? <label style={{ color: 'red' }}  >CANCELED</label> :
                                    ...
                                    i.OSN == 'Accepted' ? <label style={{ color: '#3899EC' }} >ACCEPTED</label> :
                                      i.OSN == 'PaymentFailed' ? <Stack direction='row' alignItems='center' justifyContent='space-between' ><label style={{ color: 'red' }}  >Payment Failed</label>
                                        ...
                                        i.DOS == '13' ? <label style={{ color: 'green' }} >Delivered</label> :
                                          i.OSN == 'Delivered' ?
                                            <>
                                              <table style={{ width: '100%' }}>
                                                <tr>
                                                  <td>
                                                    <label style={{ color: '#3899EC' }} >On The Way</label>
                                                  </td>
